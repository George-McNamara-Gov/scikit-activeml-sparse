...
class TestDatasets(unittest.TestCase):
    ...
    @unittest.skipIf(not successful_skorch_torch_import, reason="torch not installed")
    def setUp(self):
    ...
    def test_cache_numpy_dataset(self):
        for n_samples in [1, 10, 100]:
            ...
            with self.subTest(msg="raw-sample mode"):
                ...
                np.testing.assert_array_equal(X_raw_1.squeeze(), np.arange(n_samples, dtype=np.float32))
                np.testing.assert_array_equal(y_raw_1, np.arange(n_samples, dtype=np.int64))
                X_raw_2, y_raw_2 = cache_numpy_dataset(
                    dataset=TensorPairDataset(n_samples),  # fresh dataset instance
                    model_fn=None,
                    batch_size=4,
                    cache_dir=".",
                    verbose=False,
            ...
            with self.subTest(msg="embedding mode"):
                ...
                np.testing.assert_array_equal(X_emb_1.shape, (n_samples, 1))  # identity keeps original shape
            ...
            with self.subTest(msg="hugginface-like mode"):
                ...
                self.assertRaises(TypeError, cache_numpy_dataset, dataset=dict_ds,
                        model_fn=None,
            ...
            with self.subTest(msg="list dataset"):
                ...
                np.testing.assert_array_equal(X_list_1.shape, n_samples)  # identity keeps original shape
            ...
            with self.subTest(msg="heterogeneous dataset"):
                ...
                if n_samples > 1:
                    self.assertEqual(X_heterogeneous_1.dtype, object)  # identity keeps original shape
                ...
                for i in range(len(X_heterogeneous_2)):
                    np.testing.assert_array_equal(X_heterogeneous_1[i], X_heterogeneous_2[i])

        with self.subTest(msg="check number of cached files"):
            # finally, ensure exactly two cache files exist (raw + identity)
            cached_files = [p for p in  Path(".").iterdir() if p.suffix == ".npz"]
