...
class SingleAnnotatorWrapper(MultiAnnotatorPoolQueryStrategy):
    ...
    def _query_annotators(
        ...
        s_indices, s_utilities = self._get_order_preserving_s_query(
            A_cand, sample_utilities, annotator_utilities, qs_indices, self.random_state_
        )
    ...
    def _get_order_preserving_s_query(
        A, candidate_utilities, annotator_utilities, sample_indices, random_state
    ):
        ...
        rand_permutation = random_state.permutation(candidate_utilities.shape[1])
